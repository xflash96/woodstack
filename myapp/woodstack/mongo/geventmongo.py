...
class GPool(object):
    ...
    def _bootstrap(self, pid):
        ...
        self._used = {}
        self._ref = {}
        self._queue = gevent.queue.Queue(self.max_size)
    ...
    def connect(self, host, port):
        ...
        try:
            ...
            s.settimeout(self.net_timeout or self.conn_timeout)
            s.connect((host, port))
        ...
        if self.use_ssl:
            raise NotImplementedError('ssl not implemented')
    ...
    def get_socket(self, host, port):
        ...
        if sock is None:
            with self._lock:
                if self._count < self.max_size: #FIXME
                    self._count += 1
                    from_pool = False
                    sock = self.connect(host, port)
        if sock is None:
            from_pool = True
            sock = self._queue.get(timeout=self.net_timeout)
    ...
    def _return(self, greenlet):
        try:
            ...
            ref = self._ref.get(greenlet)
            if ref is not None:
                del self._ref[greenlet]
        except IndexError:
            with self._lock:
                self._count -= 1
...
def patch():
    ...
    print 'pymongo patched with gevent'
